...
```{r}
lapse_filter <- function(subid, context, lapses) {
  
  context_tmp <- context |> filter(subid == subid)
  
  lapses_tmp <- lapses |> filter(subid == subid)
...
context_filtered <- context$subid |>
  unique() |>
  furrr::future_map(\(subid) lapse_filter(subid, context, lapses)) |>  
  list_rbind() |> 
